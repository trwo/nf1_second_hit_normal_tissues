setwd("/lustre/scratch126/casm/team274sb/to3/paed_brain/autopsy_reanalysis/wgs_data/02_DNA_processing/variant_calls/01_subs/09_swater")
...
for(sample in manifest_flt$sample){
  data = read.table(paste0(sample, ".standard.gnomAD.AF.0.001.PON.read.direction.low_cov.bq.mq.binom.5bp.indel.swater.filtered.txt"), header = T, sep = '\t', stringsAsFactors = F)
...
table(all_subs$Mutant) #T/TRUE issues

all_subs[all_subs$Mutant == "TRUE",]$Mutant = "T" #correct issue where R handles T base as Boolean
...
write.table(all_subs, "all_sample_subs_distribution_annotated_20240302.txt", col.names = T, row.names = F, sep = "\t", quote = F)