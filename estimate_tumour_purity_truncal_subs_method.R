...
### PD51122 ###
patient = "PD51122"
truncal_cluster = "6_15" #merged

manifest = read.table('/lustre/scratch126/casm/team274sb/to3/paed_brain/autopsy_reanalysis/wgs_data/00_supplementary_data/wgs_paeds_pm_sample_metadata_20240225.txt', header = T, sep = '\t', stringsAsFactors = F, quote = '')
...
vars = read.table(paste0('/lustre/scratch126/casm/team274sb/to3/paed_brain/autopsy_reanalysis/wgs_data/03_DNA_analyses/nd_dpclust/01_Input/', patient, '/md_out/', patient, '_allClusterassignmentsFromParallelRuns_clusters_reviewed.txt'), header = T, sep = '\t', stringsAsFactors = F) #reviewed and merged clusters
...
dp_files = list.files(paste0('/lustre/scratch126/casm/team274sb/to3/paed_brain/autopsy_reanalysis/wgs_data/03_DNA_analyses/nd_dpclust/01_Input/', patient), "allDirichletProcessInfo_fromMultipleSamplesWithoutChromosomeLosses.txt", full.names = T)
...
length(hi_conf_loci) #52

#read in VAF dataframe containing every substitution (rows) and its VAF across all samples (columns) from the case
VAF = read.table(paste0("/lustre/scratch126/casm/team274sb/to3/paed_brain/autopsy_reanalysis/wgs_data/02_DNA_processing/merged_subs/", patient, "_VAF_final_merged_snps.txt"), header = T, sep = "\t", stringsAsFactors = F)
...
#get variants in trunk
vars = read.table(paste0('/lustre/scratch126/casm/team274sb/to3/paed_brain/autopsy_reanalysis/wgs_data/03_DNA_analyses/nd_dpclust/01_Input/', patient, '/md_out/', patient, '_allClusterassignmentsFromParallelRuns.txt'), header = T, sep = '\t', stringsAsFactors = F) #from original as root unchanged
...
#get variants in trunk
vars = read.table(paste0('/lustre/scratch126/casm/team274sb/to3/paed_brain/autopsy_reanalysis/wgs_data/03_DNA_analyses/nd_dpclust/01_Input/', patient, '/md_out/', patient, '_allClusterassignmentsFromParallelRuns.txt'), header = T, sep = '\t', stringsAsFactors = F)
cluster_vars = vars[vars$cluster.no == truncal_cluster,]
cluster_vars$ID = paste0(cluster_vars$chr, "_", cluster_vars$pos)
...
write.table(manifest, '/lustre/scratch126/casm/team274sb/to3/paed_brain/autopsy_reanalysis/wgs_data/00_supplementary_data/wgs_paeds_pm_sample_metadata_20240225.txt', sep = '\t', quote = F, col.names = T, row.names = F)