setwd("/lustre/scratch126/casm/team274sb/to3/paed_brain/autopsy_reanalysis/wgs_data/02_DNA_processing/variant_calls/02_indels/05_indel")
...
indel_files = list.files(".", ".pindel.HP.9.read.direction.germline.beta.binom.unk.amb.min.depth.5bp.indel.filtered.txt")

all_indels = data.frame()
for(file in indel_files){
  
  sample = unlist(strsplit(file, ".pindel.HP.9.read.direction.germline.beta.binom.unk.amb.min.depth.5bp.indel.filtered.txt"))
...
all_indels[all_indels$Wildtype == "TRUE",]
all_indels[all_indels$Mutant == "TRUE",] #no T/TRUE issues
...
write.table(all_indels, "all_sample_indels_distribution_annotated_20240302.txt", col.names = T, row.names = F, sep = "\t", quote = F)